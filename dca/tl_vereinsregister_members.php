...
(
	...
	(
		...
		(
			...
			(
				...
				'button_callback'     => array('tl_vereinsregister_members', 'toggleIcon')
			),
			'show' => array
			(
	...
	(
		...
		'title' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['title'],
			'exclude'                 => true,
			'search'                  => true,
			'inputType'               => 'text',
			'eval'                    => array('maxlength'=>255, 'tl_class'=>'w50'),
		...
		'size' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['size'],
			'exclude'                 => true,
			'inputType'               => 'imageSize',
		...
		'imagemargin' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['imagemargin'],
			'exclude'                 => true,
			'inputType'               => 'trbl',
		...
		'protected' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['protected'],
			'exclude'                 => true,
			'filter'                  => true,
			'inputType'               => 'checkbox',
			'eval'                    => array('submitOnChange'=>true),
		...
		'groups' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['groups'],
			'exclude'                 => true,
			'inputType'               => 'checkbox',
		...
		'guests' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['guests'],
			'exclude'                 => true,
			'filter'                  => true,
			'inputType'               => 'checkbox',
		...
		'cssID' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['cssID'],
			'exclude'                 => true,
			'inputType'               => 'text',
		...
		'space' => array
		(
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['space'],
			'exclude'                 => true,
			'inputType'               => 'text',
		...
		(
			'exclude'                 => true,
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['start'],
			'inputType'               => 'text',
		...
		(
			'exclude'                 => true,
			'label'                   => &$GLOBALS['TL_LANG']['tl_vereinsregister_members']['stop'],
			'inputType'               => 'text',
			'eval'                    => array('rgxp'=>'datim', 'datepicker'=>true, 'tl_class'=>'w50 wizard'),
			'sql'                     => "varchar(10) NOT NULL default ''"
		),
...
class tl_vereinsregister_members extends Backend
{
	...
	public function listHistory($arrRow)
	{
		...
		$temp = '<div class="cte_type ' . $key . '">Mitglied' . ($arrRow['protected'] ? ' (' . $GLOBALS['TL_LANG']['MSC']['protected'] . ')' : ($arrRow['guests'] ? ' (' . $GLOBALS['TL_LANG']['MSC']['guests'] . ')' : '')) . '</div>';
	...
	public function toggleIcon($row, $href, $label, $title, $icon, $attributes)
	{
		...
		if (!$this->User->isAdmin && !$this->User->hasAccess('tl_vereinsregister_members::invisible', 'alexf'))
		{
			return '';
		}
	...
	public function toggleVisibility($intId, $blnVisible)
	{
		...
		if (is_array($GLOBALS['TL_DCA']['tl_vereinsregister_members']['config']['onload_callback']))
		{
			foreach ($GLOBALS['TL_DCA']['tl_vereinsregister_members']['config']['onload_callback'] as $callback)
			{
		...
		$objVersions = new Versions('tl_vereinsregister_members', $intId);
		$objVersions->initialize();

		// Trigger the save_callback
		if (is_array($GLOBALS['TL_DCA']['tl_vereinsregister_members']['fields']['invisible']['save_callback']))
		{
			foreach ($GLOBALS['TL_DCA']['tl_vereinsregister_members']['fields']['invisible']['save_callback'] as $callback)
			{
				if (is_array($callback))
				{
					$this->import($callback[0]);
		...
		// Update the database
		$this->Database->prepare("UPDATE tl_vereinsregister_members SET tstamp=". time() .", invisible='" . ($blnVisible ? '' : 1) . "' WHERE id=?")
					   ->execute($intId);

		$objVersions->create();
		$this->log('A new version of record "tl_vereinsregister_members.id='.$intId.'" has been created'.$this->getParentEntries('tl_vereinsregister_members', $intId), __METHOD__, TL_GENERAL);
	} 
